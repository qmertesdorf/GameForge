...
2. **Derive the concept.** From the prompt, decide:
   ...
   - **TRADEOFF GATE — the #1 cause of "weak but functional" games. A *described* decision is not a *real* one.** Naming a choice is worthless if one option dominates: the loop is then *solved*, and it plays as going-through-the-motions no matter how well it runs or how much juice it has. The high-value action MUST **sacrifice** something the player also wants, so the optimal play is never obvious. **Apply the test before writing `core_loop`:** state the single move a skilled player makes each beat; if that move is *both the most rewarding AND the safest* (or otherwise strictly best), there is **no decision — reject the design and add a cost.** Two shipped POC games failed exactly here: shopkeep-0001's "triage a patient queue" was pure icon-matching with no penalty for serve order (best = serve anyone), and match3-survival-0003's "score-vs-survive sacrifice" was inert because purging the threat *also scored 5× a normal clear* (reward and safety aligned). Encode the tension in `core_loop` as an explicit clause — **"doing X gains A but gives up B"** — where A (reward) and B (safety / a competing reward / a future resource) genuinely pull in opposite directions. Common engines for this: push-your-luck (bank now vs. risk the haul for more), resource contention (one pool feeds offense OR defense, never both), opportunity cost (the high-score move exposes you / costs tempo), or time pressure (the thorough move is the slow move). If you cannot name B, the design is not ready for `builder`.
