...
- **Icon master** — a **bespoke transparent focal** generated via `tools/comfy.mjs` + the asset checkpoint with **LayerDiffuse** (RGBA), square (1024²), recorded as `store_pass.icon_master` (e.g. `store/icon_focal.png`). Use the icon prompt scaffold verbatim:
  ...
  > single bold focal subject · centered, filling the frame · **glossy, dimensional, 3D-style cartoon** · soft studio lighting with a **bright highlight + soft shadow** · rich saturated colour · simple iconic silhouette · NO text · NO words · NO scene/background · NO multiple objects — plus the title's own hero subject from `concept.theme`, square (1024²).

  **Aim for the app-store look, not flat in-game art.** Top casual-game icons read as a glossy "mini-poster": one dominant subject, dimensional rendering (highlights + soft shadow), vibrant colour, depth from a gradient/glow background — NOT the flat cel-shaded look of the in-game sprites (a flat scaffold yields a flat, un-app-storey icon; the dogfood needed `glossy · dimensional · 3d-style · studio lighting · highlight` to land it). The icon is allowed to be more polished/glossy than the game's own art — it's the store face. (`comfy.mjs` checkpoint must be the **exact filename** incl. `.safetensors`, e.g. `cartoonArcadiaXL_v2.safetensors` — a newer ComfyUI rejects the bare name.)
