...
## Precondition — the game must already be DEEP (carry a `depth_pass`)
**Do not re-skin a prototype.** A `playable` game with **no `manifest.depth_pass`** has never been through `deepen` — it is a first-playable prototype (solved loop, content ceiling a minute in), and polishing it just produces a prettier weak game (the POC's #1 failure: ten of eleven titles re-skinned/packaged without a single depth pass). **Check `manifests/<id>.json` for a `depth_pass` block first.** If it is absent, STOP and hand the title to `deepen` — a re-skin is the reward for a game that's actually good to play, not a substitute for making it good. (A deliberately-trivial arcade toy may be exempt, but say so explicitly and record the waiver in `asset_pass.notes`.)

## Inputs
- `manifests/<id>.json` with a populated `concept` block, `status = "playable"`, **and a `depth_pass`** (see precondition above).
